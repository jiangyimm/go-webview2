...
import (
	...
	"github.com/jiangyimm/go-webview2/internal/w32"
	"github.com/jiangyimm/go-webview2/pkg/edge"
...
func (w *webview) CreateWithOptions(opts WindowOptions) bool {
	...
	if opts.Center {
		...
		screenWidth, _, _ := w32.User32GetSystemMetrics.Call(w32.SM_CXSCREEN)
		//screenHeight, _, _ := w32.User32GetSystemMetrics.Call(w32.SM_CYSCREEN)
		posX = uint(screenWidth) - opts.Right - windowWidth
		posY = opts.Top
	}
...
func (w *webview) SetSize(width int, height int, top int, right int, hints Hint) {
	index := w32.GWLStyle
	...
	if hints == HintMax {
		...
		screenWidth, _, _ := w32.User32GetSystemMetrics.Call(w32.SM_CXSCREEN)
		r := w32.Rect{}
		r.Left = int32(int(screenWidth) - width - right)
		r.Top = int32(top)
		r.Right = int32(width)
		...
		_, _, _ = w32.User32SetWindowPos.Call(
			w.hwnd, 0, uintptr(r.Left), uintptr(r.Top), uintptr(r.Right), uintptr(r.Bottom),
